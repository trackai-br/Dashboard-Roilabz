...
export default function CampaignWizardPage() {
  ...
  // Initialize account from query param if provided
  React.useEffect(() => {
    if (router.query.accountId && !formData.accountId) {
      dispatch({
        type: 'SET_ACCOUNT',
        payload: {
          accountId: router.query.accountId as string,
          accountName: router.query.accountName as string || 'Selected Account',
        },
      });
      setCurrentStep(1);
    }
  }, [router.query, formData.accountId]);
            ...
            {currentStep < 5 && (
              <div className="mt-8 flex justify-between pt-6 border-t border-gray-200">
                <button
                  onClick={() => {
                    if (currentStep === 1) {
                      // Going back from step 1, return to central
