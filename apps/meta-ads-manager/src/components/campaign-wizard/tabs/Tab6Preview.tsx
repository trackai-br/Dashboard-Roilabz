...
export default function Tab6Preview({ onGoToTemplate }: Tab6PreviewProps) {
      ...
      <div className="space-y-6">
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-sm font-bold" style={{ color: 'var(--color-text-primary)', ...headingFont }}>
              {publishDone ? 'Publicacao Concluida' : `Publicando: ${publishCompleted} de ${total} campanhas`}
            </h3>
            <span className="text-sm font-bold" style={{ color: 'var(--color-accent)', ...headingFont }}>{pct}%</span>
          </div>
          ...
          <div className="space-y-4">
            <div className="p-4 rounded-lg" style={{ backgroundColor: 'rgba(255,255,255,0.03)', border: '1px solid var(--color-border)' }}>
              <div className="grid grid-cols-3 gap-4 text-center">
                <div>
                  <p className="text-lg font-bold" style={{ color: 'var(--color-accent)', ...headingFont }}>{successCount}</p>
                  <p className="text-xs" style={{ color: 'var(--color-text-tertiary)' }}>Publicadas</p>
                </div>
                <div>
                  <p className="text-lg font-bold" style={{ color: failedCount > 0 ? 'var(--color-danger)' : 'var(--color-text-tertiary)', ...headingFont }}>{failedCount}</p>
                  <p className="text-xs" style={{ color: 'var(--color-text-tertiary)' }}>Com erro</p>
                </div>
                <div>
                  <p className="text-lg font-bold" style={{ color: 'var(--color-accent)', ...headingFont }}>{formatCurrency(totalBudget * (successCount / Math.max(total, 1)))}</p>
                  <p className="text-xs" style={{ color: 'var(--color-text-tertiary)' }}>Orcamento ativado</p>
                </div>
              </div>
            </div>
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Distribution Table */}
      <div>
        <h3 className="text-sm font-bold mb-3" style={{ color: 'var(--color-text-primary)', ...headingFont }}>Tabela de Distribuicao</h3>
            ...
            <thead>
              <tr style={{ backgroundColor: 'rgba(255, 255, 255, 0.04)' }}>
                {([['campaign', 'Campanha'], ['account', 'Conta'], ['page', 'Pagina'], ['adsets', 'Adsets'], ['status', 'Status']] as [SortKey, string][]).map(([key, label]) => (
                  <th key={key} onClick={() => handleSort(key)} className="px-3 py-2 text-left cursor-pointer select-none whitespace-nowrap" style={{ color: 'var(--color-text-secondary)', fontWeight: 600, borderBottom: '1px solid var(--color-border)' }}>
                    {label}
                    {sortKey === key && <span className="ml-1">{sortDir === 'asc' ? '\u25B2' : '\u25BC'}</span>}
                  </th>
                ))}
                <th className="px-3 py-2 text-left" style={{ color: 'var(--color-text-secondary)', fontWeight: 600, borderBottom: '1px solid var(--color-border)' }}>Tipos de Adset</th>
              </tr>
            </thead>
            <tbody>
              {sortedRows.length === 0 && (
                <tr><td colSpan={6} className="px-3 py-4 text-center" style={{ color: 'var(--color-text-tertiary)' }}>Nenhuma distribuicao configurada</td></tr>
              )}
              {sortedRows.map((row, i) => (
                <tr key={i} style={{ borderBottom: '1px solid var(--color-border)' }}>
                  ...
                  <td className="px-3 py-2 text-xs" style={{ color: 'var(--color-text-tertiary)' }}>{row.types}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="space-y-2">
        ...
        <AccordionSection id="adset" title="Configuracoes - Adsets" open={openSections.has('adset')} onToggle={() => toggleSection('adset')}>
          {state.adsetTypes.length === 0 ? (
            <p className="text-sm" style={{ color: 'var(--color-text-tertiary)' }}>Nenhum tipo de adset configurado.</p>
          ) : (
        ...
        <AccordionSection id="ad" title="Configuracoes - Anuncios" open={openSections.has('ad')} onToggle={() => toggleSection('ad')}>
          {!ad ? (
            <p className="text-sm" style={{ color: 'var(--color-text-tertiary)' }}>Anuncio nao configurado.</p>
          ) : (
            ...
            <div className="relative rounded-xl p-6 border shadow-xl max-w-lg w-full" style={{ backgroundColor: '#1a1a2e', borderColor: willSpendMoney ? 'rgba(255, 51, 51, 0.5)' : 'rgba(57, 255, 20, 0.3)' }}>
              ...
              <div className="space-y-2 mb-4">
                <div className="flex justify-between text-sm">
                  <span style={{ color: 'var(--color-text-secondary)' }}>Campanhas</span>
                  <span className="font-semibold" style={{ color: 'var(--color-text-primary)' }}>{state.totalCampaigns}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span style={{ color: 'var(--color-text-secondary)' }}>Adsets totais</span>
                  <span className="font-semibold" style={{ color: 'var(--color-text-primary)' }}>{totalAdsets}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span style={{ color: 'var(--color-text-secondary)' }}>Contas</span>
                  <span className="font-semibold" style={{ color: 'var(--color-text-primary)' }}>{state.selectedAccountIds.length}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span style={{ color: 'var(--color-text-secondary)' }}>Orcamento diario total</span>
                  <span className="font-bold" style={{ color: willSpendMoney ? '#ff5555' : 'var(--color-accent)' }}>{formatCurrency(totalBudget)}</span>
                </div>
                <div className="flex justify-between text-sm">
              ...
              <div className="flex gap-3 justify-end pt-2 border-t" style={{ borderTopColor: 'var(--color-border)' }}>
                <button
  ...
  return (
    <div className="rounded-lg border overflow-hidden" style={{ borderColor: open ? 'rgba(57, 255, 20, 0.2)' : 'var(--color-border)', backgroundColor: 'rgba(255, 255, 255, 0.02)' }}>
      <button onClick={onToggle} className="w-full flex items-center gap-2 px-4 py-3 text-left">
        ...
        <span className="text-sm font-bold" style={{ color: 'var(--color-text-primary)', fontFamily: "var(--font-sans)" }}>{title}</span>
      </button>
      {open && (
        <div className="px-4 pb-4 border-t" style={{ borderTopColor: 'var(--color-border)' }}>
...
function InfoRow({ label, value, small }: { label: string; value: string; small?: boolean }) {
  return (
    <div>
      ...
      <span className={small ? 'text-xs' : 'text-sm'} style={{ color: 'var(--color-text-primary)' }}>{value}</span>
    </div>
  );
}
    ...
    <div>
      <div
        ...
        style={{
          ...
          border: `1px solid ${isFailed ? 'rgba(255, 51, 51, 0.2)' : 'var(--color-border)'}`,
          cursor: isFailed && cataloged ? 'pointer' : 'default',
        }}
        onClick={() => { if (isFailed && cataloged) setExpanded(!expanded); }}
      >
        <span className="text-sm">{statusIcon}</span>
        <span className="flex-1 text-sm truncate" style={{ color: 'var(--color-text-primary)' }}>{cpName}</span>
        <span className="text-xs max-w-[200px] truncate" style={{ color: statusColor }}>{statusText}</span>
        {isFailed && cataloged && (
          <svg
            width="14" height="14"
            className="flex-shrink-0 transition-transform"
            style={{ color: 'var(--color-text-tertiary)', transform: expanded ? 'rotate(180deg)' : 'rotate(0)' }}
      ...
      {expanded && isFailed && cataloged && (
        ...
        >
          <div>
            <span className="font-bold" style={{ color: 'var(--color-text-primary)' }}>{cataloged.title}</span>
          ...
          {result?.error && result.error !== cataloged.title && (
            <div className="pt-1 border-t" style={{ borderTopColor: 'rgba(255,255,255,0.05)' }}>
              <span className="font-medium" style={{ color: 'var(--color-text-tertiary)' }}>Mensagem original: </span>
              <span style={{ color: 'var(--color-text-tertiary)' }}>{result.error}</span>
            </div>
          )}
